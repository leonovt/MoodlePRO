from pathlib import Path

import httpx
import pytest

import app.services.transcribe_groq as tg
from app.core.config import settings
from app.services.srt import Segment


class _FakeResponse:
    def __init__(self, status_code: int, json_body: dict | None = None, headers: dict | None = None):
        self.status_code = status_code
        self._json = json_body or {}
        self.headers = headers or {}

    def json(self) -> dict:
        return self._json

    def raise_for_status(self) -> None:
        if self.status_code >= 400:
            raise httpx.HTTPStatusError(f"HTTP {self.status_code}", request=None, response=None)


class _FakeClient:
    """Async-context-manager stand-in for httpx.AsyncClient that replays queued responses."""

    def __init__(self, responses):
        self._responses = list(responses)
        self.post_calls = 0

    async def __aenter__(self):
        return self

    async def __aexit__(self, *exc):
        return False

    async def post(self, *args, **kwargs):
        response = self._responses[self.post_calls]
        self.post_calls += 1
        return response


def test_offset_segments_shifts_timestamps():
    segments = [Segment(text="a", start=0.0, end=2.0), Segment(text="b", start=2.0, end=4.0)]
    shifted = tg._offset_segments(segments, 600.0)
    assert [(s.start, s.end) for s in shifted] == [(600.0, 602.0), (602.0, 604.0)]
    assert [s.text for s in shifted] == ["a", "b"]  # text untouched


async def test_small_file_is_single_request(monkeypatch, tmp_path):
    audio = tmp_path / "small.wav"
    audio.write_bytes(b"x" * 1024)

    calls: list[Path] = []

    async def fake_one(self, path, language):
        calls.append(path)
        return [Segment(text="hello", start=0.0, end=1.0)], "hello"

    def fake_split(*args, **kwargs):  # must NOT be called for a small file
        raise AssertionError("small file should not be chunked")

    monkeypatch.setattr(tg.GroqTranscriber, "_transcribe_file", fake_one)
    monkeypatch.setattr(tg, "_split_audio", fake_split)

    tr = tg.GroqTranscriber(api_key="k", max_upload_mb=1.0)  # 1 MB limit; file is 1 KB
    result = await tr.transcribe(audio, "he")

    assert calls == [audio]  # the original file, sent once
    assert result.text == "hello"


async def test_large_file_is_chunked_and_stitched(monkeypatch, tmp_path):
    audio = tmp_path / "big.wav"
    audio.write_bytes(b"x" * 4096)

    # Three 10-minute chunks at offsets 0, 600, 1200.
    def fake_split(path, chunk_seconds, dest_dir):
        return [(0.0, dest_dir / "c0"), (600.0, dest_dir / "c1"), (1200.0, dest_dir / "c2")]

    async def fake_one(self, path, language):
        # one segment per chunk, local time 0..2, text tagged by chunk file name
        return [Segment(text=f"seg-{path.name}", start=0.0, end=2.0)], ""

    monkeypatch.setattr(tg, "_split_audio", fake_split)
    monkeypatch.setattr(tg.GroqTranscriber, "_transcribe_file", fake_one)

    tr = tg.GroqTranscriber(api_key="k", max_upload_mb=0.000001, chunk_seconds=600.0)
    result = await tr.transcribe(audio, "he")

    # all three chunks stitched, in order
    assert result.text == "seg-c0 seg-c1 seg-c2"
    # timestamps offset onto the original timeline: 600s -> 00:10:00, 1200s -> 00:20:00
    assert "00:10:00,000 --> 00:10:02,000" in result.srt
    assert "00:20:00,000 --> 00:20:02,000" in result.srt
    assert result.language == "he"


def test_retry_delay_prefers_retry_after_header():
    tr = tg.GroqTranscriber(api_key="k")
    resp = _FakeResponse(429, headers={"retry-after": "5"})
    assert tr._retry_delay(resp, attempt=0) == 5.0


def test_retry_delay_falls_back_to_exponential_backoff(monkeypatch):
    monkeypatch.setattr(settings, "groq_retry_base_seconds", 2.0)
    monkeypatch.setattr(settings, "groq_retry_max_seconds", 60.0)
    tr = tg.GroqTranscriber(api_key="k")
    no_header = _FakeResponse(429)
    assert tr._retry_delay(no_header, attempt=0) == 2.0   # 2 * 2^0
    assert tr._retry_delay(no_header, attempt=2) == 8.0   # 2 * 2^2
    assert tr._retry_delay(no_header, attempt=10) == 60.0  # capped


async def test_transcribe_retries_on_429_then_succeeds(monkeypatch, tmp_path):
    audio = tmp_path / "small.wav"
    audio.write_bytes(b"x" * 64)

    responses = [
        _FakeResponse(429, headers={"retry-after": "0"}),
        _FakeResponse(200, {"text": "after retry", "segments": []}),
    ]
    fake_client = _FakeClient(responses)
    monkeypatch.setattr(tg.httpx, "AsyncClient", lambda *a, **k: fake_client)

    async def _no_sleep(_seconds):
        return None

    monkeypatch.setattr(tg.asyncio, "sleep", _no_sleep)

    tr = tg.GroqTranscriber(api_key="k", max_upload_mb=1.0, max_retries=4)
    result = await tr.transcribe(audio, "he")

    assert fake_client.post_calls == 2  # one 429, one success
    assert result.text == "after retry"


async def test_transcribe_raises_after_exhausting_retries(monkeypatch, tmp_path):
    audio = tmp_path / "small.wav"
    audio.write_bytes(b"x" * 64)

    fake_client = _FakeClient([_FakeResponse(429, headers={"retry-after": "0"})] * 3)
    monkeypatch.setattr(tg.httpx, "AsyncClient", lambda *a, **k: fake_client)

    async def _no_sleep(_seconds):
        return None

    monkeypatch.setattr(tg.asyncio, "sleep", _no_sleep)

    tr = tg.GroqTranscriber(api_key="k", max_upload_mb=1.0, max_retries=2)
    with pytest.raises(httpx.HTTPStatusError):
        await tr.transcribe(audio, "he")

    assert fake_client.post_calls == 3  # initial + 2 retries, all 429
